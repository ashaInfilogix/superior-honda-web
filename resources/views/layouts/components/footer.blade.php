...
<footer class="footer__section footer__bg">
            ...
            <div class="row ">
                ...
                <div class="col-lg-2 col-md-4">
                    <div class="footer__widget">
                        ...
                        <ul class="footer__widget--menu footer__widget--inner">
                            <?php
                               $categories = App\Models\ProductCategory::latest()->take(3)->get();
                            ?>
                            @foreach($categories as $key => $category)
                                <li class="footer__widget--menu__list"><a class="footer__widget--menu__text" href="#">{{ $category->name }}</a></li>
                            @endforeach
                            <li class="footer__widget--menu__list"><a class="footer__widget--menu__text" href="{{ route('accesories') }}">Accessories</a></li>
                            <li class="footer__widget--menu__list"><a class="footer__widget--menu__text" href="{{ route('products.index') }}">Products</a></li>
                        </ul>
                    </div>
                </div>
            </div>
   ...
   $(function() {
        ...
        $(document).on('submit', '.cart-submit', function(event) {
            ...
            var actionValue;
            if ($(this).find('.quickview__cart--btn').is(':focus')) {
                actionValue = 'add-to-cart';
            } else if ($(this).find('.variant__buy--now__btn').is(':focus')) {
                actionValue = 'buy-it-now';
            } else {
                actionValue = 'add-to-cart'; // Default to add-to-cart if neither button is focused (fallback)
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success == true) {
                        ...
                        if (response.action == 'buy-it-now') {
                            window.location.href = '{{ route('checkout') }}'
                        }
                        if(response.viewProduct == 'view-product') {
                            $('.quantityNumber').val(response.product_quantity);
                        }
                    } else {
                        alert('Error');
                    }
